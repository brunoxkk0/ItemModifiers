...
import io.github.adainish.itemmodifiers.config.ShinyConfig;
...
public class Shiny {
    ...
    public static void loadItems() {
        items.clear();
        CommentedConfigurationNode rootNode = ShinyConfig.getConfig().get().node();
    ...
    public static class Items {
        ...
        public Items(String key) {
            this.setKey(key);
            this.setEnchantItem(ShinyConfig.getConfig().get().node(key, "Enchanted").getBoolean());
            this.setAllowDitto(ShinyConfig.getConfig().get().node(key, "AllowDitto").getBoolean());
            this.setAllowLegends(ShinyConfig.getConfig().get().node(key, "AllowLegends").getBoolean());
            this.setAllowUltraBeasts(ShinyConfig.getConfig().get().node(key, "AllowUBs").getBoolean());
            this.setItemType(ShinyConfig.getConfig().get().node(key, "ItemString").getString());
            this.setPermission(ShinyConfig.getConfig().get().node(key, "Permission").getString());
            this.setDisplay(ShinyConfig.getConfig().get().node(key, "Display").getString());
            try {
                this.setSpecFlags(ShinyConfig.getConfig().get().node(key, "Specs").getList(TypeToken.get(String.class)));
            } catch (SerializationException e) {
                e.printStackTrace();
            }
            try {
                this.setLore(ShinyConfig.getConfig().get().node(key, "Lore").getList(TypeToken.get(String.class)));
            } catch (SerializationException e) {
                e.printStackTrace();
            }
        ...
        public ItemStack getItem() {
            ...
            stack.getTag().putString("itemType", "Shiny");
            return stack;
        }
