...
public class IVSConfig extends Configurable {
    private static IVSConfig config;

    public static IVSConfig getConfig() {
        if (config == null) {
            config = new IVSConfig();
        }
        return config;
    }
    ...
    public void populate() {
        ...
        try {
            ...
            this.get().node("Example", "Lore").set(Arrays.asList("&cOmg, An Item", "&b&lAnd it modifies my pokemons IVS!")).comment("the Lore displayed on the Item");
            this.get().node("Example", "Display").set("&dIV Transformer");
            ...
            this.get().node("Example", "Amount").set(1).comment("How many IVS are we working with?");
